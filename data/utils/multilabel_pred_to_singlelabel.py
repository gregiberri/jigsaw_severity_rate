...
def single_object_multilabel_pred_to_single_label(pred, labels_names, positive_threshold):
    ...
    max_pred, max_pred_i = torch.max(pred, dim=-1)
    ...
    single_label_preds[min_pred > positive_threshold] = int(np.argwhere(labels_names == 'mindket_cica'))
    single_label_preds[max_pred < positive_threshold] = int(np.argwhere(labels_names == 'egyik_sem'))
...
def multilabel_pred_to_single_label(pred, labels_names, positive_threshold):
    ...
    # get the maximum pred (if it is under the threshold then it is no_object)
    max_pred, max_pred_i = torch.max(pred, dim=-1)
